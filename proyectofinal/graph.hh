...
public:
  ...
  */
  ...
  int inDegree3(){//O(2|V|+|E|)
    unordered_map<V,int> inDegreeVertex;
    for(const V& n: getNodes()){//O(|V|)
      inDegreeVertex[n]=0;
    }
    for(const V& n:getNodes()){//O(|E|)
      for(const V& v:adj[n]){
        inDegreeVertex[v]+=1;
      }
    }
...
void bfs(const AdjList<V, W>& g, const V& start,
         unordered_map<V, bool>& visited) {//O(|E|+|V|)
  queue<V> q;
  q.push(start);
  visited[start] = true;
  while (!q.empty()) {
...
unordered_map< V , int> connectedComponents (const AdjList<V,W> &g,   pair<int,int> &cb , int &i = 1){
	...
	for(const V &v: g.getNodes()){components[v]= 0;  visited[v] = false;} //inicializao las tablas O(|V|) 
	//int i = 1;//el valor de la primera componente conexa
	while(moreToVisit(visited)){ //O(|V|)
		V n = ChooseNext(visited);//O(|V|)
		dfs(g, n ,visited);//O(|E|+|V|)
		findComponents(visited, components, i , cb);//O(|V|)
    //cout << i << endl;
		i++;
  ...
  while(!q.empty()){
    ...
    if(!visited[n]){
      ...
      for(const V& i: g.getAdjacentTo(n)){//O(|E|)
        if(!visited[i]){
        W dist = distancias[n] + g.getWeight(n,i);  
          if(distancias[i] > dist){
            distancias[i] = dist;
            padre[i]= n;
            q.push(distancias[i],i);//O(|V|*log2(|V|))
          }  
        }
      }
    }
...
W Diameter( AdjList<V,W>& g, const int &BiggerCc , unordered_map< V , int>& ComponentC ){
  ...
  for(const V& i : g.getNodes()){//O(|V|)
    if(ComponentC[i] == BiggerCc)
      dijkstraDiameter(g,distancias,padre,visited,i,&Diam); //O(|E|+|V|*log2(|V|))
...
template<typename V,typename W>
 void MstKruskal( AdjList<V,W>& g, const unordered_map<pair<V,V>,W, WeightHash<V>, EdgeEqual<V>> &weights
  ...
  while(!q.empty()){
    ...
    if(forest.FindSet(p) != forest.FindSet(s)){//O(LOG(|V|))
      //pair<W,pair<V,V>>
       mst.insert({p,s});
      forest.Union(p,s);
    }
 ...
 int KargerCut( AdjList<V,W>& g, const unordered_map<pair<V,V>,W, WeightHash<V>, EdgeEqual<V>> &weights
  ...
  for(const V& v: g.getNodes()){
    if(ComponentC[v] == MaxComponetC.first){
      forest.MakeSet(v);
    }//Es mejor recibirlo para disminuir complejidad
  }
...
int MinCut( AdjList<V,W>& g,const unordered_map<pair<V,V>,W, WeightHash<V>, EdgeEqual<V>> &weights
  ...
  for (int i = 0 ; i < pow(MaxComponetC.second,2) ; i++){//O(|V|^2)
    int n = KargerCut(g,weights,ComponentC , MaxComponetC,rEngine ,dist);//(2E*LOG(E)+LOG(|V|)+|V|+|E|)
    if(n < resMincut){
      resMincut =n;
    }
    if(resMincut == 1) return resMincut;//LA TRIVIAL
  }
  return resMincut;
}
