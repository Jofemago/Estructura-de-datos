...
template<typename V,typename W>
unordered_map<int,W>  Diameter( AdjList<V,W>& g, const int& NumOfCc ,const pair<int,int>& MaxComponetC , unordered_map< V , int>& ComponentC ){
  unordered_map<V,bool> visited;
  unordered_map<V,int> distancias;
  unordered_map<V,int> padre;
  unordered_map<int,W> diameterCC;
  ...
  for(const V& i : g.getNodes()){//O(|V|)
    //que si el nodo pertenece a la componente conexa que lo calcule 
    dijkstraDiameter(g,distancias,padre,visited,i,&diameterCC[ComponentC[i]]); 
  }
  return diameterCC;
}

template<typename V,typename W>
W Diameter2( AdjList<V,W>& g, const int &BiggerCc , unordered_map< V , int>& ComponentC ){
  unordered_map<V,bool> visited;
  unordered_map<V,int> distancias;
  unordered_map<V,int> padre;
  //tabla int y int
